...
<style>
    ...
    .kp-subtitle { font-size:15px; font-weight:700; color:#1e2a4a; margin:24px 0 14px; }
</style>

<a href="{{ route('admin.kms.kategori.show', $subkategori->kategori_id) }}" class="kp-back">&larr; Kembali ke {{ $subkategori->kategori->nama }}</a>

<div class="kp-header">
    <h1>{{ $subkategori->nama }}</h1>
    <div>
        <a href="{{ route('admin.kms.grup.create', $subkategori) }}" class="kp-btn kp-btn-outline">+ Grup Dokumen</a>
        <a href="{{ route('admin.kms.dokumen.create', $subkategori) }}" class="kp-btn kp-btn-primary">+ Tambah Dokumen</a>
    </div>
</div>

<p class="kp-subtitle">Grup Dokumen</p>
<div class="kp-card">
        ...
        <tbody>
            @forelse ($subkategori->grupDokumens as $grup)
                <tr>
                    ...
                    <td>
                        <a href="{{ route('admin.kms.grup.show', $grup) }}" class="kp-btn kp-btn-outline">Kelola</a>
                        <a href="{{ route('admin.kms.grup.edit', $grup) }}" class="kp-btn kp-btn-outline">Edit</a>
                        <form action="{{ route('admin.kms.grup.destroy', $grup) }}" method="POST" onsubmit="return confirm('Hapus grup ini beserta dokumennya?');" style="display:inline">
                            @csrf @method('DELETE')
            ...
            @empty
                <tr><td colspan="3" class="kp-empty">Belum ada grup dokumen.</td></tr>
            @endforelse
        </tbody>
    </table>
</div>

<p class="kp-subtitle">Dokumen Langsung (tanpa grup)</p>
<div class="kp-card">
    <table class="kp-table">
        <thead>
            <tr>
        ...
        <tbody>
            ...
            @empty
                <tr><td colspan="4" class="kp-empty">Belum ada dokumen langsung.</td></tr>
            @endforelse
        </tbody>
    </table>
</div>
